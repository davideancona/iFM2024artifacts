...
trace_expression('Main', Main) :- (Main=((relevant_et>>(((((NoMultipleSubs/\NoMultipleNewPub)/\((subsOrRecv_et>>SubsThenRecv);1))/\((newPubOrPub_et>>NewPubThenPub);1))/\CheckSubs)/\CheckPub));1)),
	...
	(GenCheckSubs=gen([subId, topic], ((unsubs_et(var(subId), var(topic))*1)\/(star(notNewPub_et)*optional(var(pubId, (newPub_et(var(pubId))*(((involve_et(var(pubId), var(subId), var(topic))>>app(Queue, [var(pubId), var(subId), var(topic)]));1)/\app(GenCheckSubs, [var(subId), var(topic)]))))))))),
	...
	(Queue=gen([pubId, subId, topic], ((unsubs_et(var(subId), var(topic))*1)\/optional(var(msgId, (pub_et(var(pubId), var(topic), var(msgId))*((recv_et|app(Queue, [var(pubId), var(subId), var(topic)]))/\((recv_et>>(recv_et(var(subId), var(topic), var(msgId), var(pubId))*1));1)))))))).
